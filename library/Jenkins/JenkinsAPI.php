...
/**
 * Jenkins API
 * originally from https://github.com/uglide/jenkins-php-api
 * Original Author: https://github.com/srogier
 */

class Jenkins_JenkinsAPI
{
     ...
     * @param $token
     */
    public function __construct($baseUrl, $userName, $token)
    {
    ...
    private function _setCredentials(&$curl)
    {
        curl_setopt($curl, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
        curl_setopt($curl,CURLOPT_USERPWD, $this->userName . ":" . $this->token);
    }

    private function _getCURL($url, array $options = array())
    {
        $curl = curl_init($url);

        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);

        $this->_setCredentials($curl);

        if (!empty($options)) {
            foreach ($options as $optKey => $optVal) {
                curl_setopt($curl, $optKey, $optVal);
            }
        }
    ...
    {
        $curl = $this->_getCURL($this->baseUrl . '/api/json');
    ...
    public function getExecutors($computer = '(master)')
    {
        ...
        for ($i = 0; $i < $this->jenkins->numExecutors; $i++) {
            ...
            $executors[] = new Jenkins_JenkinsAPI_Executor($infos, $computer, $this);
        }
    ...
    public function getJob($jobName)
    {
        ...
        return new Jenkins_JenkinsAPI_Job($info, $this);
    }
    ...
    public function getQueue()
    {
        ...
        return new Jenkins_JenkinsAPI_Queue($infos, $this);
    }

    /**
     * @return Jenkins_JenkinsAPI_View[]
     */
    ...
    /**
     * @return Jenkins_JenkinsAPI_View|null
     */
    ...
    public function getView($viewName)
    {
        ...
        return new Jenkins_JenkinsAPI_View($infos, $this);
    }
     ...
     * @param string $tree
     *
     * @return Jenkins_JenkinsAPI_Build
     * @throws RuntimeException
     */
    public function getBuild($job, $buildId, $tree = 'actions[parameters,parameters[name,value]],result,duration,timestamp,number,url,estimatedDuration,builtOn')
    {
        ...
        return new Jenkins_JenkinsAPI_Build($infos, $this);
    }
    ...
    public function getComputer($computerName)
    {
        ...
        return new Jenkins_JenkinsAPI_Computer($infos, $this);
    }
    ...
    public function setJobConfig($jobname, $configuration)
    {
        ...
        $curl = $this->_getCURL(
            $url, array(
                CURLOPT_POST => 1,
                CURLOPT_POSTFIELDS => $configuration
            )
        );
    ...
    /**
     * @param Jenkins_JenkinsAPI_Executor $executor
     * @throws RuntimeException
     */
    public function stopExecutor(Jenkins_JenkinsAPI_Executor $executor)
    {
    ...
    /**
     * @param Jenkins_JenkinsAPI_JobQueue $queue
     *
     * @throws RuntimeException
     * @return void
     */
    public function cancelQueue(Jenkins_JenkinsAPI_JobQueue $queue)
    {
    ...
    public function getTestReport($jobName, $buildId)
    {
        ...
        return new Jenkins_JenkinsAPI_TestReport($this, $infos, $jobName, $buildId);
    }
